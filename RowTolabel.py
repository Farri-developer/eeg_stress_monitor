...
def calculate_stress_score(row):
    ...
    # Simple stress score combining EEG Beta/Alpha, PPG_HR, HRV (PPG_RMSSD)
...
def run_extraction():
    ...
    try:
        ...
        # ---------- ASSIGN STRESS LABELS BASED ON QUANTILES ----------
        final_df['Stress_Label'] = pd.qcut(final_df['StressScore'], q=3,
                                           labels=['0', '1', '2'])
...
# GUI
root = tk.Tk()
root.title("EEG + PPG + BP + HRV + Stress Label Extractor")
...
tk.Button(root, text="Run Feature Extraction", command=run_extraction,
          bg="green", fg="white", height=2, width=40).pack(pady=30)
